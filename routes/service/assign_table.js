...
const Bill = require('../../modules/service/bill');
...
router.post('/assign-table', authMiddleware, async (req, res) => {
    try {
...
router.post('/bills/table/:table_number/add-items', authMiddleware, async (req, res) => {
    try {
...
router.patch('/bills/:bill_id/settle', authMiddleware, async (req, res) => {
    try {
        const bill = await Bill.findById(req.params.bill_id);
        if (!bill) return res.status(404).json({ message: 'Bill not found' });

        if (bill.status === 'cleared') return res.status(400).json({ message: 'Bill is already cleared' });
        ...
        res.status(200).json({ message: 'Bill settled and table marked as vacant', bill });
    } catch (err) {
        console.error(err.message);
...
router.get('/bills/table/:table_number', authMiddleware, async (req, res) => {
    try {
        const { table_number } = req.params;
        const table = await Table.findOne({ table_number });
        if (!table) {
